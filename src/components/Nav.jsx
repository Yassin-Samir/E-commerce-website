...
const CartSelector = createSelector(
  ...
  ({ cart }) =>
    cart.length ? cart.reduce((acc, ind) => acc + ind.quantity, 0) : null
);
function CartComponent({ displayNav, Nav }) {
  const numOfItems = useSelector(CartSelector);
  return (
    <>
...
function Nav() {
  ...
  return (
    <nav className={style.nav}>
      ...
      <CartComponent displayNav={displayNav} Nav={Nav} />
    </nav>
  );
}
export default Nav;
